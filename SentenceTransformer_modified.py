...
class SentenceTransformer(nn.Sequential):
    ...
    def fit(self,
        ...
        random_list = list(range(num_train_objectives))
        ...
        for epoch in trange(epochs, desc="Epoch", disable=not show_progress_bar):
            ...
            for _ in trange(steps_per_epoch, desc="Iteration", smoothing=0.05, disable=not show_progress_bar):
                ...
                for train_idx in range(num_train_objectives):
                    train_idx = random_list[train_idx]
                    ...
                    if use_amp:
                        ...
                        # 在每一次迭代中，每一个训练目标可计算一个损失值
                        value = value + loss_value.item()
                ...
                # 把这个epoch中，所有的迭代轮次的损失装在一个列表中
                one_epoch_list.append(value)
            ...
            one_epoch_loss_item = sum(one_epoch_list)/num_train_steps
            print("第 {i} 个epoch的损失为 {j}".format(i=clock, j=one_epoch_loss_item))
            clock = clock + 1
            loss_value_list.append(one_epoch_loss_item)
