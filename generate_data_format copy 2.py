...
data_list = []
data_path = 'D:\Python4work\paper_modified_test\gte_ft_data_dev.jsonl'
...
path = 'D:\Python4work\paper_modified_test\gte_dev_data'


def generate_the_query_jsonl(processed_data, path):
    corpus_save_path = os.path.join(path, 'query.jsonl')
    i = 0
    with open(corpus_save_path, 'w', encoding='utf-8') as file_obj:
        for item in processed_data:
            query_text = ' '.join(item['query'])
            json_data = json.dumps({'_id':'query_{a}'.format(a=i),'text': query_text,'title':'', 'metadata':'', 'processed_data_index':i}, ensure_ascii=False)
...
def generate_hard_negatives_jsonl(processed_data, path):
    ...
    with open(hard_save_path, 'w', encoding='utf-8') as file_obj:
        for i in range(len(processed_data)):
            qid = 'query_{a}'.format(a=i)
            diag_seen_slots = list(processed_data[i]['dialog_seen'].keys())
            refer_slot = list(processed_data[i]['corpus'].values())
    ...
    with open(qrels_tsv_path, 'w', newline='') as file_obj:
        for i in range(len(processed_data)):
            for k,v in processed_data[i]['corpus'].items():
                all_positive_data.append(['query_{a}'.format(a=i), slot_des2corpus_id[v], '1'])
