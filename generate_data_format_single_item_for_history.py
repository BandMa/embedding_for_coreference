...
data_list = []
data_path = 'D:\Python4work\paper_modified_test\\all_history_raw_data\gte_ft_data_train.jsonl'
...
for i in range(len(data_list)):
    history = []
    for turn in data_list[i]['history'][:]:
...
path = 'D:\Python4work\paper_modified_test\\all_history_raw_data\\train_t2_all\\train_t2_all'
    ...
    with open(corpus_save_path, 'w', encoding='utf-8') as file_obj:
        for item in processed_data:
            for k,v in item['corpus'].items():
                #b = 'which slot has the same value with {a}'.format(a=k)
