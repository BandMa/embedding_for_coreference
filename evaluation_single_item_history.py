...
for i in range(len(data_list)):
    history = []
    for turn in data_list[i]['history'][:turn_cut]:
        usr_utt = ' '.join(turn['usr'])
        sys_utt = ' '.join(turn['sys'])
        history.append(sys_utt+' '+usr_utt)
...
info_list = []
def evaluate(model_save_path, model_id, info_list):
    model_path = os.path.join(model_save_path, model_id)
    ...
    for i in range(len(processed_data)):
        ...
        for wait4fill, known in processed_data[i]['corpus'].items():
            ...
            query_embedding = model.encode(' '.join(processed_data[i]['query']+[b]))
            ...
            result_sorted = [i[0] for i in result_sorted][0]
            result_list.append({'test':[result_sorted], 'gold':[processed_data[i]['corpus'][wait4fill]]})
            u = u + 1
    ...
    info_text = '模型id为{a}，测试总数为：{all_num}，其完全正确数：{cn}，正确比为：{ratio}'.format(a=model_id, all_num=u,cn=correct_one, ratio=correct_one/u)
    print(info_text)
    info_list.append(info_text)

"""
model_save_path = 'D:\Python_Program\pretrained_model\gte-base-en'
test_id_list = [str(88+i*88) for i in range(20)]
for i in test_id_list:
    evaluate(model_save_path, i, info_list)
"""
evaluate('D:\Python4work', 'gte-tiny', info_list)
